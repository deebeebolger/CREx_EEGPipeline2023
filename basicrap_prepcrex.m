...
function [winrej, chanrej] = basicrap_prepcrex(EEG, chanArray, ampth, windowms, stepms, firstdet, fcutoff, forder, thresholdType, numChanThreshold)

winrej  = [];
chanrej = [];
if nargin<9
        numChanThreshold = floor(size(EEG.data,1)/5);
end

if nargin<9
        thresholdType = 'peak-to-peak';
end
if nargin<8
        forder = 26;
end
